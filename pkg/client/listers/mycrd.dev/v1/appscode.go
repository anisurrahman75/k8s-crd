...
type AppsCodeLister interface {
	...
	List(selector labels.Selector) (ret []*v1.AppsCode, err error)
...
// List lists all AppsCodes in the indexer.
func (s *appsCodeLister) List(selector labels.Selector) (ret []*v1.AppsCode, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.AppsCode))
	})
	return ret, err
}
...
type AppsCodeNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.AppsCode, error)
...
// List lists all AppsCodes in the indexer for a given namespace.
func (s appsCodeNamespaceLister) List(selector labels.Selector) (ret []*v1.AppsCode, err error) {
...
// Get retrieves the AppsCode from the indexer for a given namespace and name.
func (s appsCodeNamespaceLister) Get(name string) (*v1.AppsCode, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("appscode"), name)
	}
	return obj.(*v1.AppsCode), nil
}
